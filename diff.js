...
const dataProvider = (fileName) => {
    const localResponsePath = `${goLocalPath}/${fileName}`
    const liveResponsePath = `${goLivePath}/${fileName}`
    ...
    diffAdm = modifiedKeys.reduce((acc, key) => {
        ...
        if (isPrimitive(v1) && isPrimitive(v2)) {
            if (v1 !== v2) {
                ...
                acc.modified = [...acc.modified, fullKey + ' ' + o1[key] + ' ' + o2[key]]
            }
            return acc
        } else {
...
const writeChangesToFile = (fileName, added, modified, deleted) => {
    ...
    `;
    fs.writeFileSync(`${diffFilePath}/${fileName}`, data)
};

var arguments = process.argv.splice(2);
var traceId = arguments[0];
const [localData, liveData] = dataProvider(traceId)
...
if (!isEmpty(added) || !isEmpty(modified) || !isEmpty(deleted)) {
    writeChangesToFile(traceId, added, modified, deleted)
}